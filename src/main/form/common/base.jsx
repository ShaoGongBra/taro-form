...
export default ({ data = {}, children }) => {
  ...
  return <View className='form-base' style={{ ...config.compStyle, ...data.style }}>
    {!config.hideText && !!data.text && <Text
      style={{ ...config.compTextStyle, ...data.compTextStyle }}
      className='form-base__text'
    >{data.text}</Text>}
    <View className='form-base__main'>
      {children}
      {!!data.verifyMsg && <Text className='form-base__verify'>{data.verifyMsg}</Text>}
      {!config.hideTip && !!data.tip && <Text
        className='form-base__tip'
        style={{ ...config.compTipStyle, ...data.compTipStyle }}
      >{data.tip}</Text>}
    </View>
  </View>
}
