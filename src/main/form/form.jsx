...
export default class FormComponent extends Component {
  ...
  /**
   * 记录条件显示 这里记录的都是key
   * 下面的示例表示 control1这个key控制input这个key的显示和隐藏，control1这个key控制input2和input3这两个个key的显示和隐藏
  ...
  getForm() {
    return (Promise.resolve()).then(() => {
      ...
      const getFormKeyIndex = (form = res.form, indexs = []) => {
      ...
      Object.keys(this.formKeyIndex).map(key => {
        const item = this.formKeyIndex[key]
      ...
      this.setState({
        form: res.form,
        values: getFormDefaultValue(res.form, res.values),
        config: res.config || {},
      }, () => this.checkShow())
    })
  }
    ...
    for (const valueKey in keys) {
      if (keys.hasOwnProperty(valueKey)) {
        for (let i = 0; i < keys[valueKey].length; i++) {
          ...
          // 被控制的表单项目
          const whereItem = this.getKeyComp(whereKey, form)
  ...
  checkVerify(key) {
    const { form } = this.state
    const compItem = this.getKeyComp(key, form)
   ...
   * 通过key获取到组件内容
   * @param {string} key
   * @param {object} form 表单组件 传入form返回的值是引用的， 可以在修改后直接 setState
   */
  getKeyComp(key, form = this.state.form) {
    return recursionGetValue(this.formKeyIndex[key].map(item => item[0]), form, 'child')
  }
  ...
  render() {
    const { form, config, values } = this.state
    return <Create form={form} config={config} values={values} onEvent={this.event.bind(this)} />
  }
}
